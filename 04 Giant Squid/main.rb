...
class Board
  ...
  def play(numbers, timeout: Float::INFINITY)
    numbers.each do |number|
      break if @numbers_processed >= timeout
...
shortest_play_duration = Float::INFINITY
score = nil
...
boards.map { |b| b.drop(1) } # Dispose leading slice separation (blank line)
      ...
      .each do |board|
  b = Board.new(board)
  play_duration = b.play(numbers, timeout: shortest_play_duration)

  next unless play_duration.finite?

  if play_duration < shortest_play_duration
    shortest_play_duration = play_duration
